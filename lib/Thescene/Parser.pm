...
my %REGULAR_EXPRESSIONS=(
  ...
  title => [qr/^(.*?)\.\./],
  language => [
    qr/\.\.(MULTi|DANiSH|SWEDiSH|NORWEGiAN|GERMAN|iTALiAN|iTA\.|FRENCH|RUSSIAN|SPANiSH|PORTUGUESE|ENGLiSH|FiNNiSH|GREEK|DUTCH)\.*(?!sub)/i,
    qr/\.([A-Za-z]{2,3}[\.-]audio)\./i
  ],
  subtitles => [
    qr/\.([a-z]{2,3}[\.-]*sub(bed|s)?)\./i,
    qr/\.((ingebakken|DANiSH|SWEDiSH|NORWEGiAN|GERMAN|iTALiAN|FRENCH|RUSSIAN|SPANiSH|PORTUGUESE|ENGLiSH|FiNNiSH|GREEK|DUTCH)\.*sub(s)*)\./i,
    qr/\.(HC)\./i
    ],
